...
def weighted_accuracy(pred, labels):
    correct_per_class = Counter()
    total_per_class = Counter()

    pred_labels = pred.view(-1).cpu().numpy()
    ...
    accuracy_per_class = {c: (correct_per_class[c] / total_per_class[c]) for c in total_per_class}
    weighted_acc = sum(accuracy_per_class.values()) / len(accuracy_per_class)
    return weighted_acc
...
def test(args, model, device, train_graphs, test_graphs, num_class):
    ...
    wa_train = weighted_accuracy(pred_, labels)
    ...
    wa_test = weighted_accuracy(pred, labels)


    print("accuracy train: %f test: %f" % (acc_train, acc_test))

    print("weighted accuracy train: %f test: %f" % (wa_train, wa_test))

    return acc_train, acc_test,wa_train, wa_test, output, labels
...
def main():
    ...
    parser.add_argument('--graph_pooling_type', type=str, default="sum", choices=["sum", "mean", "max"],
                        help='Pooling over nodes in a graph to get graph embeddig: sum, mean or max')
    ...
    for i in range(args.fold_idx):
        ...
        wa_train_sum += wa_train*len(train_data)
        total_train_samples += len(train_data)

        wa_test_sum += wa_test*len(test_data)
    ...
    print(
        'Weighted average train acc: %f,  Weighted average test acc: %f' % (wa_train_sum / total_train_samples, wa_test_sum / total_test_samples))
